...
const CampusVerificationSystem: React.FC = () => {
  ...
  const allCampusImages = [
    'campus1.jpg', 'campus2.jpg', 'campus3.jpg', 'campus4.jpg', 
    'campus5.jpg', 'campus6.jpg', 'campus7.jpg', 'campus8.jpg',
    'campus9.jpg', 'campus10.jpg'
  ];
  
  const allExternalImages = [
    'external1.jpg', 'external2.jpg', 'external3.jpg', 'external4.jpg',
    'external5.jpg', 'external6.jpg', 'external7.jpg', 'external8.jpg',
    'external9.jpg', 'external10.jpg', 'external11.jpg', 'external12.jpg',
    'external13.jpg', 'external14.jpg', 'external15.jpg', 'external16.jpg'
  ];
  ...
  const startTimer = useCallback(() => {
    ...
    const timer = setTimeout(() => {
      setTimeRemaining(prev => {
        if (prev <= 1) {
          showModalDialog('Time Expired', `Level ${currentLevel} failed. Score: ${totalPoints}`, () => resetVerification());
          return 0;
        }
        return prev - 1;
      });
  ...
  const toggleSelection = (index: number) => {
    setSelectedImages(prev => {
      if (currentLevel === 1) {
        // Level 1: only one selection allowed
        return prev.includes(index) ? [] : [index];
      } else {
        // Level 2 & 3: multiple selections
  ...
  const validateSelection = () => {
    ...
    if (correctSelections === required && incorrectIndices.length === 0) {
      if (currentLevel < 3) {
        showModalDialog('Success!', `Level ${currentLevel} complete. Points: ${levelPoints}`, () => {
          setCurrentLevel(prev => prev + 1);
        });
      } else {
        showModalDialog('Verification Complete!', `Final score: ${totalPoints + levelPoints}`, () => resetVerification());
      }
    } else {
      showModalDialog('Verification Failed', `Incorrect. Score: ${totalPoints + levelPoints}`, () => resetVerification());
    }
  };
  ...
  const showModalDialog = (title: string, message: string, callback?: () => void) => {
    setModalTitle(title);
      ...
      <div className="max-w-4xl w-full bg-white rounded-lg shadow-lg p-5">
        ...
        {!imagesLoaded && !loadingError && (
          <div className="text-center py-8">
            <div className="text-lg text-gray-600 mb-4">Loading images...</div>
        ...
        {loadingError && (
          <div className="bg-red-50 border-l-4 border-red-400 p-4 mb-5">
            ...
            <div className="text-red-600 text-sm mt-1">{loadingError}</div>
            <button
        ...
        {/* Game Content - Only show when images are loaded */}
        {imagesLoaded && !loadingError && (
          <>
            {/* Info Bar */}
            <div className="flex justify-between items-center mb-5 p-3 bg-gray-50 rounded">
              ...
              <div className={`text-lg font-bold ${isWarningTime() ? 'text-red-600 animate-pulse' : 'text-red-500'}`}>
                ...
                <div
                  ...
                  className={`
                    ...
                    ${selectedImages.includes(index) 
                      ...
                      : 'border-gray-300'}
                    ${currentLevel === 1 ? 'w-48 h-48' : currentLevel === 2 ? 'w-36 h-36' : 'w-28 h-28'}
                  `}
                  onClick={() => toggleSelection(index)}
                >
                  <img
                    src={imgObj.name}
                    alt={imgObj.isCampus ? 'Campus image' : 'External image'}
                  ...
                  />
                  <div 
                    className="w-full h-full bg-gradient-to-br from-blue-100 to-purple-100 flex items-center justify-center text-gray-600 font-medium"
                    style={{ display: 'none' }}
                  >
